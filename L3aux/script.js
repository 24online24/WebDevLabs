...
const demoShopType = document.querySelector("#demo-shop-type");
const demoDaysType = document.querySelector("#demo-days-type");
const demoCategory = document.querySelector("#demo-category");
const demoVisible = document.querySelector("#demo-visible");
const demoTotal = document.querySelector("#demo-total");
const demoTheme = document.querySelector("#demo-theme");
...
function updateConceptPanel(lastEvent, visibleCount) {
  const currentTheme = document.body.classList.contains("dark-theme")
    ? "dark"
    : "light";

  demoShopType.textContent = `${shopName} (${typeof shopName})`;
  demoDaysType.textContent = `${maxAdvanceDays} (${typeof maxAdvanceDays})`;
  demoCategory.textContent = activeCategory;
  demoVisible.textContent = String(visibleCount);
  demoTotal.textContent = String(menuItems.length);
  demoTheme.textContent = currentTheme;
  demoEvent.textContent = lastEvent;
}
...
function setFilter(category) {
  ...
  const filteredItems = getFilteredItems(menuItems, activeCategory);
  renderMenu(filteredItems);
  updateConceptPanel("Category filter changed", filteredItems.length);
}
...
function setupReservationForm() {
  reservationForm.addEventListener("submit", (event) => {
    ...
    if (validationError) {
      ...
      updateConceptPanel(
        "Reservation submit blocked by validation",
        getFilteredItems(menuItems, activeCategory).length,
      );
      return;
    }
...
async function loadMenuData() {
  try {
    ...
    renderMenu(filteredItems);
    updateConceptPanel("Menu JSON loaded", filteredItems.length);
  } catch (error) {
    menuGrid.innerHTML =
      '<p class="menu-status">Menu unavailable right now. Please refresh and try again.</p>';
    updateConceptPanel("Menu JSON failed to load", 0);
    console.error(error);
  }
}
...
function initializeApp() {
  ...
  updateConceptPanel("App initialized", 0);
  loadMenuData();
}
